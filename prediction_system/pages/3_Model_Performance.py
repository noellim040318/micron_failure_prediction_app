...
feature importance, SHAP summary plots, and threshold calibration analysis.
"""
...
tab_roc, tab_pr, tab_cm, tab_thresh, tab_imp, tab_shap = st.tabs([
    ...
    "Threshold Calibration", "Feature Importance", "SHAP Summary"
])
...
# ── SHAP Summary ──────────────────────────────────────────────────────────────
with tab_shap:
    st.subheader("SHAP Global Summary (Training Sample)")
    ...
    def compute_shap_plot(seed: int = 42):
        ...
        bar_bytes = shap_engine.bar_figure(X_s, max_display=20)
        bee_bytes = shap_engine.beeswarm_figure(X_s, max_display=20)
        return bar_bytes, bee_bytes

    bar_b, bee_b = compute_shap_plot()
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Mean |SHAP| (feature impact)**")
