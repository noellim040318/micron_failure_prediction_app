...
st.caption("Enter HBM packaging process parameters for one unit and receive an instant risk score with full SHAP attribution.")
...
if arts is not None and any(c in arts.get("feature_cols", []) for c in _OLD_COLS):
    if not st.session_state.get("_schema_cache_cleared"):
...
with st.expander("📋 Process Parameters (expand to edit)", expanded=True):
    ...
    with tabs[1]:  # TCB BONDING — Thermal Compression Bonding (HBM stacking interconnect)
        ...
        c1, c2, c3 = st.columns(3)
        tcb_force   = c1.number_input("⚠️ TCB Bond Force (N)", 150.0, 500.0, float(defaults.get("tcb_bond_force_n", 300)), step=5.0)
        tcb_temp    = c2.number_input("⚠️ TCB Bond Temperature (°C)", 220.0, 290.0, float(defaults.get("tcb_bond_temp_c", 250)), step=1.0)
        tcb_time    = c3.number_input("TCB Bond Time (s)", 3.0, 15.0, float(defaults.get("tcb_bond_time_s", 8.0)), step=0.5)
        c4, c5, c6 = st.columns(3)
        tcb_mis     = c4.number_input("⚠️ Die Misalignment (μm)", 0.0, 5.0, float(defaults.get("tcb_misalignment_um", 1.0)), step=0.1)
