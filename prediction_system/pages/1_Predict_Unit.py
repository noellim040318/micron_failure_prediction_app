"""
Single-unit failure prediction with Trajectory Shapley Attribution and
engineer-readable risk report with HOLD / MONITOR / CONTINUE MES disposition.
"""
...
if run:
    ...
    # ── Risk headline + MES disposition ──────────────────────────────────────
    if exp["risk_level"] == "HIGH":
        st.error(
            f"{risk_color} {exp['headline']}  \n"
            "**MES DISPOSITION: HOLD** — Do not release to downstream processing. "
            "Route unit for SAM (Scanning Acoustic Microscopy) and post-bond electrical "
            "characterisation. Await engineer sign-off before lot release."
        )
    elif exp["risk_level"] == "MEDIUM":
        st.warning(
            f"{risk_color} {exp['headline']}  \n"
            "**MES DISPOSITION: MONITOR** — Flag unit in MES for enhanced inspection at "
            "the next quality gate. Increase lot sampling rate until root cause is confirmed."
        )
    else:
        st.success(
            f"{risk_color} {exp['headline']}  \n"
            "**MES DISPOSITION: CONTINUE** — Unit cleared for downstream processing. "
