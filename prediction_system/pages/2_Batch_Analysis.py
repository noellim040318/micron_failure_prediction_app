...
if "batch_result" in st.session_state:
    ...
    # Keep all columns for TSA, then subset for display
    top20 = result_df.nlargest(20, "risk_score").reset_index(drop=True)

    # ── TSA 1-sentence explanation per unit ───────────────────────────────────
    from src.explainability.translator import FEATURE_CONTEXT
    _tsa = arts["tsa_engine"]
    _prep = arts["prep"]
    _fcols = arts["feature_cols"]
    try:
        _X = _prep.transform(top20, _fcols)
        _sv = _tsa.explain(_X)
        _why = []
        for i in range(len(top20)):
            row = top20.iloc[i]
            sv_dict = dict(zip(_fcols, _sv[i]))
            feat = max(sv_dict, key=lambda f: sv_dict[f])
            ctx = FEATURE_CONTEXT.get(feat, {})
            name = ctx.get("name", feat.replace("_", " ").title())
            val = row[feat] if feat in top20.columns else None
            unit_str = ctx.get("unit", "")
            nominal = ctx.get("nominal")
            if val is not None and nominal is not None:
                pct = (val - nominal) / abs(nominal) * 100
                risk_txt = (
                    (ctx.get("high_risk") if pct > 0 else ctx.get("low_risk"))
                    or ctx.get("high_risk") or ctx.get("low_risk") or "Risk driver."
                )
                direction = "above" if pct > 0 else "below"
                _why.append(
                    f"{name} = {val:.1f} {unit_str} ({abs(pct):.0f}% {direction} nominal): {risk_txt}"
                )
            elif val is not None:
                risk_txt = ctx.get("high_risk") or ctx.get("low_risk") or "Risk driver."
                _why.append(f"{name} = {val:.2f} {unit_str}: {risk_txt}")
            else:
                _why.append(f"{name}: primary TSA risk driver (+{sv_dict[feat]:.3f}).")
    except Exception:
        _why = ["Explanation unavailable."] * len(top20)

    top20_display = top20[display_cols].copy()
    top20_display["Explanation"] = _why
    ...
    fmt = {k: v for k, v in {
        ...
        "mold_cure_time_s": "{:.0f}",
    }.items() if k in top20_display.columns}

    st.dataframe(
        top20_display.style
            .map(lambda v: (
                "background-color: #fadbd8" if isinstance(v, float) and v >= 0.6
                else "background-color: #fdebd0" if isinstance(v, float) and v >= arts["optimal_threshold"]
                else ""
            ), subset=["risk_score"] if "risk_score" in top20_display.columns else [])
            .format(fmt),
