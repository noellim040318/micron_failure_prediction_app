...
def main():
    ...
    st.info(
        ...
        **Deployment point:** Line-side industrial edge controller at the Thermal Compression Bonding (TCB)
        station — representative equipment: Besi TCB bonder, ASMPT AMICRA die/flip-chip bonder.
        The controller connects to equipment via SECS/GEM, OPC UA, or MQTT and issues
        **HOLD / MONITOR / CONTINUE** dispositions directly to the Manufacturing Execution System (MES)
        without replacing existing packaging equipment.

        1. **Data Aggregation & SMOTE Engine** — collects TCB bonding force profiles, temperature ramps,
           pressure variation, tool displacement, and alignment offsets from the TCB bonder; underfill
           dispense and cure data from Nordson ASYMTEK Spectrum II S-900; reflow zone temperatures,
           ramp/cool rates, and dwell times from Heller 1913 MK7 reflow oven. Engineers 17 lineage
           features (lot context, tool PM state, production sequence). Applies SMOTE to overcome the
           <1% defect rate in fab data. Inspection feedback from Onto Innovation Dragonfly G3 and
           Nordson QUADRA 7 Pro serves as retraining labels. *(Addresses: class imbalance gap)*

        2. **Edge AI Inference Node** — XGBoost classifier running locally on the edge controller
           (zero cloud latency). Features are extracted from raw equipment waveforms: peak bond force,
           force fluctuation, temperature ramp rate, cooling rate, alignment deviation, cure duration,
           and reflow profile shift. Cost-sensitive training (FN penalty >> FP) minimises escaped
           defects. Issues HOLD / MONITOR / CONTINUE to MES. Target: < 100 ms inference per unit.
           *(Addresses: inference-latency gap)*

        3. **Trajectory Shapley Attribution Module** — TSA TreeExplainer with sequential process ordering
           and an engineer-readable translation layer mapping attributions to physics-of-failure
           explanations (interfacial delamination, microbump cracking, TSV fracture).
           *(Addresses: static-explainability gap)*

        4. **Adaptive Concept Drift Filter** — continuous PSI + KS-test monitoring across all process
           features; triggers model recalibration when equipment ageing (PM overdue), recipe revisions,
           or material lot variation shifts the process distribution. *(Addresses: concept-drift gap)*
        """
    )
