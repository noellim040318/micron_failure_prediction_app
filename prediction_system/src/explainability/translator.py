...
FEATURE_CONTEXT = {
    ...
    # WIRE BOND
    "bond_force_gf": {
        "name": "Wire Bond Force",
        "unit": "gf",
        "nominal": 20,
        "failure_link": "intermetallic bond formation at pad",
        "high_risk": "Excess bond force can cratering pad metallisation → open circuit.",
        "low_risk": "Insufficient force → incomplete ball deformation → weak bond → lift-off failure.",
    },
    "bond_power_mw": {
        "name": "Wire Bond Ultrasonic Power",
        "unit": "mW",
        "nominal": 250,
        "failure_link": "thermosonic welding energy at bond interface",
        "high_risk": "Over-power → pad damage (cratering) and substrate cracking.",
        "low_risk": "Under-power → cold bond with high resistance → early open failure.",
    },
    "wire_pull_strength_gf": {
        "name": "Wire Pull Strength",
        "unit": "gf",
        "nominal": 8.0,
        "failure_link": "bond mechanical integrity (direct measurement)",
        "high_risk": None,
        "low_risk": "Below spec pull strength is a direct indicator of weak bond → field failure risk.",
    },
    "ball_shear_gf": {
        "name": "Ball Shear Strength",
        "unit": "gf",
        "nominal": 45,
        "failure_link": "intermetallic shear strength at first bond",
        "high_risk": None,
        "low_risk": "Low shear strength signals inadequate IMC formation → bond degradation under vibration.",
    },
    # ENCAPSULATION
    "mold_temp_c": {
    ...
    "lot_std_bond_force": {
        "name": "Lot Wire Bond Force Variability",
        "unit": "gf σ",
        "nominal": None,
        "failure_link": "within-lot process stability",
        "high_risk": "High variability signals tool instability → some units have insufficient bond force.",
        "low_risk": None,
    },
    "moisture_z_in_lot": {
    ...
    "bond_force_z_in_lot": {
        "name": "Bond Force Deviation Within Lot",
        "unit": "σ",
        "nominal": 0,
        "failure_link": "anomalous bonding energy relative to lot peers",
        "high_risk": "Outlier bond force within lot → possible equipment transient → bond quality uncertainty.",
        "low_risk": None,
    },
}
...
def translate_shap_to_engineer(
    ...
    Parameters
    ----------
    top_features_df : DataFrame with columns [feature, shap_value, direction]
    ...
    top = top_features_df[top_features_df["shap_value"] > 0].head(n_reasons)
    for _, row in top.iterrows():
        ...
        elif "bond_force" in feat.lower() or "bond_power" in feat.lower():
            actions.append("Recalibrate wire bonder — check capillary wear and bond parameter recipe.")
